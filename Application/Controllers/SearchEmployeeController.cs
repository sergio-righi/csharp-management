...
{
    public partial class SearchEmployeeController : BaseController
    {
        private readonly IPersonService PersonService;

        public SearchEmployeeController(IPersonService personService)
        {
        ...
        public async virtual Task<IActionResult> Search(IFormCollection form)
        {
            ...
            ERole roleId = form.ToEnum("ddl_role", ERole.All);

            if (!string.IsNullOrWhiteSpace(name) || !string.IsNullOrWhiteSpace(document))
            {
                var people = await PersonService.GList(roleId, name, document);
                ...
                if (people != null)
                {
                    ...
                    {
                        return PartialView("~/Views/Search/Employee/_List.cshtml", people);
                    }
                    else
                    {
